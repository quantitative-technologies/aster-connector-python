...
from aster.auth import V1, V3, make_auth
...
class AsyncClientV3(AsyncAPI):
    """Asynchronous client for the Pro API (V3) surface.

    Exposes the same method names as :class:`AsyncClient` over ``/fapi/v3``
    routes, signing every authenticated request with the API wallet's key via
    EIP-712 instead of HMAC.  ``signer``/``private_key`` fall back to
    ``key``/``secret`` so a two-field credential store can drive either client.

    ``user`` is the master account wallet address.  It is optional -- the
    exchange authenticates agent-signed requests from ``signer`` alone -- and is
    carried as an opaque string, so a Solana master account works unchanged.
    """

    def __init__(self, key=None, secret=None, signer=None, private_key=None, user=None, **kwargs):
        if "base_url" not in kwargs:
            kwargs["base_url"] = "https://fapi.asterdex.com"
        kwargs.setdefault(
            "auth",
            make_auth(
                V3, key=key, secret=secret, signer=signer, private_key=private_key, user=user
            ),
        )
        super().__init__(key, secret, **kwargs)

    # MARKETS
    from aster.rest_api.market_v3_async import ping
    from aster.rest_api.market_v3_async import time
    from aster.rest_api.market_v3_async import exchange_info
    from aster.rest_api.market_v3_async import depth
    from aster.rest_api.market_v3_async import trades
    from aster.rest_api.market_v3_async import historical_trades
    from aster.rest_api.market_v3_async import agg_trades
    from aster.rest_api.market_v3_async import klines
    from aster.rest_api.market_v3_async import index_price_klines
    from aster.rest_api.market_v3_async import mark_price_klines
    from aster.rest_api.market_v3_async import mark_price
    from aster.rest_api.market_v3_async import funding_rate
    from aster.rest_api.market_v3_async import ticker_24hr_price_change
    from aster.rest_api.market_v3_async import ticker_price
    from aster.rest_api.market_v3_async import book_ticker

    # ACCOUNT(including orders and trades)
    from aster.rest_api.account_v3_async import change_position_mode
    from aster.rest_api.account_v3_async import get_position_mode
    from aster.rest_api.account_v3_async import change_multi_asset_mode
    from aster.rest_api.account_v3_async import get_multi_asset_mode
    from aster.rest_api.account_v3_async import new_order
    from aster.rest_api.account_v3_async import new_batch_order
    from aster.rest_api.account_v3_async import query_order
    from aster.rest_api.account_v3_async import cancel_order
    from aster.rest_api.account_v3_async import cancel_open_orders
    from aster.rest_api.account_v3_async import cancel_batch_order
    from aster.rest_api.account_v3_async import countdown_cancel_order
    from aster.rest_api.account_v3_async import get_open_orders
    from aster.rest_api.account_v3_async import get_orders
    from aster.rest_api.account_v3_async import get_all_orders
    from aster.rest_api.account_v3_async import balance
    from aster.rest_api.account_v3_async import account
    from aster.rest_api.account_v3_async import change_leverage
    from aster.rest_api.account_v3_async import change_margin_type
    from aster.rest_api.account_v3_async import modify_isolated_position_margin
    from aster.rest_api.account_v3_async import get_position_margin_history
    from aster.rest_api.account_v3_async import get_position_risk
    from aster.rest_api.account_v3_async import get_account_trades
    from aster.rest_api.account_v3_async import get_income_history
    from aster.rest_api.account_v3_async import leverage_brackets
    from aster.rest_api.account_v3_async import adl_quantile
    from aster.rest_api.account_v3_async import force_orders
    from aster.rest_api.account_v3_async import commission_rate

    # STREAMS
    from aster.rest_api.data_stream_listen_key_v3_async import new_listen_key
    from aster.rest_api.data_stream_listen_key_v3_async import renew_listen_key
    from aster.rest_api.data_stream_listen_key_v3_async import close_listen_key
